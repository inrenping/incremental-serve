...
CREATE TABLE public.t_log_api (
	...
	CONSTRAINT fk_log_api_user_id FOREIGN KEY (user_id) REFERENCES public.t_users(id) ON DELETE CASCADE
);

-- 表与字段的属性元数据注释
COMMENT ON TABLE public.t_log_api IS '系统级接口审计日志表';
COMMENT ON COLUMN public.t_log_api.id IS '[主键] 系统API日志全局自增大整型ID';
COMMENT ON COLUMN public.t_log_api.user_id IS '[外键] 执行操作的用户ID (t_users.id)';
