...
# --- 2. 登录接口 ---
@router.post("/login")
def login(email: str, captcha: str, request: Request, db: Session = Depends(get_db)):
    """用户登录"""
...
def github_callback(code: str, request: Request, db: Session = Depends(get_db)):
    """
...
def send_captcha(
    email: str, purpose: str, request: Request, db: Session = Depends(get_db)
):
